...
Gaussian Naive Bayes-based 7-variant benchmarking script for binary intrusion
...
All variants use a Gaussian Naive Bayes (GaussianNB) classifier as the core model
...
Features:
 ...
 - "algorithm" field saved in metrics and summary (default: "GaussianNB")
 - Optional saving (--save) and pausing between models (--pause)

Usage:
  python gnb_binary.py --csv ../SDN-Net.csv --k 30
  python gnb_binary.py --csv ../SDN-Net.csv --k 30 --save --outdir outputs --pause
...
def main(argv=None):
    ...
    parser.add_argument(
        "--algo",
        default="GaussianNB",
        help="Algorithm name label to include in results (default: GaussianNB)"
    )
