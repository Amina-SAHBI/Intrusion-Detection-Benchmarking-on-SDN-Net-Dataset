...
1D-CNN-based 7-variant benchmarking script for binary intrusion detection on the SDN-Net dataset.

This script replaces the RNN with a Conv1D (CNN) while preserving the same 7 experimental variants:
  - Unbalanced
...
Features:
 - 7 experiment variants per run (CNN)
 ...
 - "algorithm" field saved in metrics and summary (default: "CNN")
 - Optional saving (--save) and pausing between models (--pause)

Usage:
  python cnn_binary.py --csv ../SDN-Net.csv --k 30 --epochs 30 --batch_size 128 --timesteps 1 --save
...
def build_cnn_model(input_shape, filters=64, kernel_size=3, dropout=0.3, dense_units=32, lr=1e-3):
    """
    Build and compile a small 1D-CNN model for binary classification.
    input_shape: (timesteps, features_per_step)
    Note: kernel_size will be adjusted to not exceed timesteps (min = 1).
    """
    ...
    model.add(Conv1D(filters=filters // 2 if filters >= 2 else filters, kernel_size=max(1, ks - 1), activation='relu', padding='same'))
    model.add(BatchNormalization())
...
def train_cnn_display(
    ...
    """
    Train CNN model, display classification report, confusion matrix and ROC inline.
    ...
    model = build_cnn_model(input_shape, filters=filters, kernel_size=kernel_size, dropout=dropout, dense_units=dense_units, lr=optimizer_lr)
    ...
    if save_outputs:
        ...
        try:
            model_save_dir = model_dir / f"cnn_model_{stamp}"
            model.save(str(model_save_dir), include_optimizer=True)
        except Exception as e:
            # fallback to h5
            try:
                model.save(str(model_dir / f"cnn_model_{stamp}.h5"))
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="CNN", help="Algorithm name label to include in results (default: CNN)")
