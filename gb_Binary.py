...
def train_gb_display(
    ...
    """
    Train classifier (Gradient Boosting by default), display classification
    report, confusion matrix and ROC inline.

    algo_name: string to display and include in results (e.g. 'GradientBoosting')
    ...
    # Select classifier according to algo_name (Gradient Boosting-based)
    if algo_name.lower() in ("gradientboosting", "gradient_boosting", "gb", "gbdt"):
        clf = GradientBoostingClassifier(random_state=RND)
    else:
        # Fallback: GradientBoosting if unknown
        print(f"Algo '{algo_name}' not recognized; using GradientBoosting by default.")
        clf = GradientBoostingClassifier(random_state=RND)
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help="Algorithm name label to include in results (default: GradientBoosting)"
    )
