...
RNN-based 7-variant benchmarking script for binary intrusion detection on the SDN-Net dataset.

Replaces the previous KNN model with a TensorFlow/Keras RNN (LSTM by default),
while preserving the same 7 experimental pipeline variants:
  - Unbalanced
...
Features:
 - 7 experiment variants per run (now using RNN)
 ...
 - "algorithm" field saved in metrics and summary (default: "RNN")
 - Optional saving (--save) and pausing between models (--pause)

Usage:
  python rnn_binary.py --csv ../SDN-Net.csv --k 30 --epochs 30 --batch_size 128 --timesteps 1 --save
...
def build_rnn_model(input_shape, units=64, dropout=0.3, rnn_cell="lstm", dense_units=32, lr=1e-3):
    """
    Build and compile a small RNN model for binary classification.
...
def train_rnn_display(
    ...
    """
    Train RNN model, display classification report, confusion matrix and ROC inline.
    ...
    model = build_rnn_model(input_shape, units=units, dropout=dropout,
    ...
    if use_class_weight:
        try:
            ...
            # If using SMOTE balanced training, weights will be near-equal; passing class_weight is optional but harmless
        except Exception:
            class_weight = None
    ...
    if save_outputs:
        ...
        try:
            model_save_dir = model_dir / f"rnn_model_{stamp}"
            model.save(str(model_save_dir), include_optimizer=True)
        except Exception as e:
            # fallback to h5
            try:
                model.save(str(model_dir / f"rnn_model_{stamp}.h5"))
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="RNN", help="Algorithm name label to include in results (default: RNN)")
