...
def train_lr_display(
    ...
    # LogisticRegression instantiation: avoid passing n_jobs for lbfgs (not supported)
    lr_kwargs = {"C": C, "penalty": penalty, "max_iter": 1000}
    # choose solver validity: if solver is 'saga' we could add n_jobs, but keep simple
    lr_kwargs["solver"] = solver
    ...
    # Score for ROC (prefer probabilities)
    score_for_roc = None
    ...
    # Learning curve
    try:
        plot_learning_curve_for_model(LogisticRegression(C=C, penalty=penalty, solver=solver, max_iter=1000), X_train_df, y_train, title=f"Learning Curve ({name} - {algo_name})", cv=5, n_jobs=1)
    except Exception as e:
    ...
    if save_outputs:
        ...
        try:
            ...
            auc_value = None
        metrics = {"algorithm": algo_name, "C": float(C), "penalty": penalty, "solver": solver, "accuracy": float(acc), "precision": float(prec), "recall": float(rec), "f1": float(f1v), "auc": auc_value}
    ...
    return {"config": name, "algorithm": algo_name, "C": C, "penalty": penalty, "solver": solver, "accuracy": acc, "precision": prec, "recall": rec, "f1": f1v, "auc": roc_auc}
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="LogisticRegression", help="Algorithm name label to include in results (default: LogisticRegression)")
    parser.add_argument("--C", type=float, default=1.0, help="Inverse regularization strength (default: 1.0)")
    parser.add_argument("--penalty", type=str, default="l2", choices=["l1", "l2", "elasticnet", "none"], help="Penalty (default: l2)")
    parser.add_argument("--solver", type=str, default="lbfgs", help="Solver (default: lbfgs). Use 'saga' for l1/elasticnet support.")
    ...
    results.append(train_lr_display("Model1_Unbalanced_Full", X_train_raw, X_test_raw, y_train, y_test, X_full.columns, algo_name=args.algo, C=args.C, penalty=args.penalty, solver=args.solver, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))

    sel2 = rf_feature_selection(X_train_raw, y_train, args.k)
    results.append(train_lr_display("Model2_Unbalanced_RF-FS", X_train_raw[sel2], X_test_raw[sel2], y_train, y_test, sel2, algo_name=args.algo, C=args.C, penalty=args.penalty, solver=args.solver, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))

    sel3, ig_scores_full = ig_feature_selection_with_scores(X_train_raw, y_train, k=args.k)
    results.append(train_lr_display("Model3_Unbalanced_IG-FS", X_train_raw[sel3], X_test_raw[sel3], y_train, y_test, sel3, algo_name=args.algo, C=args.C, penalty=args.penalty, solver=args.solver, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    ...
    results.append(train_lr_display("Model4_SMOTE_Full", X_train_m4, X_test_raw, y_train_m4, y_test, X_full.columns, algo_name=args.algo, C=args.C, penalty=args.penalty, solver=args.solver, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))

    sel5 = rf_feature_selection(X_train_m4, y_train_m4, args.k)
    results.append(train_lr_display("Model5_SMOTE_RF-FS", X_train_m4[sel5], X_test_raw[sel5], y_train_m4, y_test, sel5, algo_name=args.algo, C=args.C, penalty=args.penalty, solver=args.solver, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))

    sel6, ig_scores_m4 = ig_feature_selection_with_scores(X_train_m4, y_train_m4, k=args.k)
    results.append(train_lr_display("Model6_SMOTE_IG-FS", X_train_m4[sel6], X_test_raw[sel6], y_train_m4, y_test, sel6, algo_name=args.algo, C=args.C, penalty=args.penalty, solver=args.solver, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    ...
    results.append(train_lr_display("Model7_SMOTETomek_Full", X_train_m7, X_test_raw, y_train_m7, y_test, X_full.columns, algo_name=args.algo, C=args.C, penalty=args.penalty, solver=args.solver, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))
