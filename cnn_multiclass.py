...
1D-CNN-based 7-variant benchmarking script for multiclass intrusion detection on the SDN-Net dataset.

This script adapts the previous cnn_binary.py to multiclass classification by:
 - using a softmax output with n_classes and sparse_categorical_crossentropy loss
 - computing multiclass metrics (accuracy, weighted precision/recall/F1)
 - computing macro ROC-AUC (OVR) when possible
 - saving class mapping (index -> label) when --save is used
...
Usage:
  python cnn_multiclass.py --csv ../SDN-Net.csv --k 30 --epochs 30 --batch_size 128 --timesteps 1 --save
...
try:
    ...
    from tensorflow.keras.layers import Conv1D, GlobalMaxPooling1D, Dense, Dropout, InputLayer, BatchNormalization
...
def build_cnn_model_multiclass(input_shape, n_classes, filters=64, kernel_size=3, dropout=0.3, dense_units=32, lr=1e-3):
    """
    Build and compile a small 1D-CNN model for multiclass classification.
    input_shape: (timesteps, features_per_step)
    """
    timesteps = input_shape[0]
    ks = min(kernel_size, timesteps) if timesteps >= 1 else 1
    if ks < 1:
        ks = 1

    model = Sequential()
    model.add(InputLayer(input_shape=input_shape))
    model.add(Conv1D(filters=filters, kernel_size=ks, activation='relu', padding='same'))
    model.add(BatchNormalization())
    model.add(Conv1D(filters=max(1, filters // 2), kernel_size=max(1, ks - 1), activation='relu', padding='same'))
    model.add(BatchNormalization())
...
def prepare_for_cnn(X_df, timesteps):
    """
    Prepare 2D dataframe X_df for CNN input.
...
def train_cnn_display_multiclass(
    name,
    X_train_df,
    X_test_df,
    y_train,
    ...
    """
    Train CNN model (multiclass), display classification report, confusion matrix and ROC inline.
    ...
    # Prepare for CNN shape
    try:
        X_train_cnn = prepare_for_cnn(pd.DataFrame(X_train_sc, columns=X_train_df.columns), timesteps)
        X_test_cnn = prepare_for_cnn(pd.DataFrame(X_test_sc, columns=X_test_df.columns), timesteps)
    except Exception as e:
        raise RuntimeError(f"Error preparing data for CNN: {e}")
    ...
    model = build_cnn_model_multiclass(input_shape, n_classes=n_classes, filters=filters, kernel_size=kernel_size,
                                       dropout=dropout, dense_units=dense_units, lr=optimizer_lr)
    ...
    y_prob = model.predict(X_test_cnn)  # shape (n_samples, n_classes)
    ...
    if save_outputs:
        ...
        try:
            model_save_dir = model_dir / f"cnn_multiclass_model_{stamp}"
            model.save(str(model_save_dir), include_optimizer=True)
        except Exception as e:
            # fallback to h5
            try:
                model.save(str(model_dir / f"cnn_multiclass_model_{stamp}.h5"))
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="CNN-Multiclass", help="Algorithm name label to include in results (default: CNN-Multiclass)")
    parser.add_argument("--no-gpu", action="store_true", help="If set, disable GPU usage (use CPU only)")
    parser.add_argument("--timesteps", type=int, default=1, help="Timesteps for CNN input (default:1). For tabular keep 1.")
    parser.add_argument("--epochs", type=int, default=30, help="Training epochs (default: 30)")
    parser.add_argument("--batch_size", type=int, default=128, help="Batch size (default: 128)")
    parser.add_argument("--patience", type=int, default=5, help="EarlyStopping patience (default: 5)")
    parser.add_argument("--filters", type=int, default=64, help="Conv1D filters (default: 64)")
    parser.add_argument("--kernel_size", type=int, default=3, help="Conv1D kernel size (default: 3)")
