...
Supervised autoencoder-based 7-variant benchmarking script for binary intrusion
detection on the SDN-Net dataset.

This script replaces the previous perceptron/MLP with a supervised autoencoder
(encoder -> latent -> classifier head) trained with a joint objective:
...
Features:
 - 7 experiment variants per run (now using supervised autoencoder)
 - Joint training (classification + reconstruction via add_loss)
 ...
 - Learning curves (training & validation loss/accuracy) from Keras history
 - "algorithm" field saved in metrics and summary (default: "Autoencoder")
 - Optional saving (--save) and pausing between models (--pause)

Usage example:
  python autoencoder_binary.py --csv ../SDN-Net.csv --k 30 --epochs 30 --batch_size 128 --latent_dim 32 --save
...
def build_supervised_autoencoder(input_dim,
    ...
    """
    Build a supervised autoencoder model:
      input -> encoder -> latent
                  -> decoder -> reconstruction (not an output)
                  -> classifier -> classification output

    The reconstruction loss is added via model.add_loss so that we can still use
    class_weight for the classification during model.fit.
    """
    ...
    classification = Dense(1, activation='sigmoid', name="classification")(latent)

    model = Model(inputs=inputs, outputs=classification, name="supervised_autoencoder")

    # Reconstruction loss added as an auxiliary loss (MSE)
    ...
    # Compile with classification loss only; total optimized loss will include the added loss
...
def train_autoencoder_display(
    ...
    """
    Train supervised autoencoder, display classification report, confusion matrix and ROC inline.
    """
    stamp = timestamp()
    ...
    # Train (note: reconstruction loss is included via model.add_loss so we can rely on class_weight)
    history = model.fit(
        X_train_p, y_train,
    ...
    if save_outputs:
        ...
        try:
            model_save_dir = model_dir / f"autoencoder_model_{stamp}"
            model.save(str(model_save_dir), include_optimizer=True)
        except Exception as e:
            # fallback to h5
            try:
                model.save(str(model_dir / f"autoencoder_model_{stamp}.h5"))
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="Autoencoder", help="Algorithm name label to include in results (default: Autoencoder)")
