...
Gaussian Naive Bayes-based 7-variant benchmarking script for multiclass intrusion detection.

Features:
 - 7 experiment variants preserved
 - trains GaussianNB for multiclass (predict_proba returns per-class probabilities)
 - metrics: accuracy, weighted precision/recall/f1, ROC macro OVR (if feasible)
 - optional sample_weight computed from class balance for fitting
...
def train_gnb_display_multiclass(
    name,
    X_train_df,
    X_test_df,
    y_train,
    ...
    features,
    class_names,
    algo_name="GaussianNB-Multiclass",
    ...
    if save_outputs:
        ...
        try:
            ...
            pass
        metrics = {"algorithm": algo_name, "use_sample_weight": bool(use_sample_weight), "n_classes": int(len(class_names)), "accuracy": float(acc), "precision_weighted": float(prec), "recall_weighted": float(rec), "f1_weighted": float(f1v), "auc_macro_ovr": float(roc_auc) if roc_auc is not None else None}
    ...
    return {"config": name, "algorithm": algo_name, "use_sample_weight": bool(use_sample_weight), "n_classes": int(len(class_names)), "accuracy": acc, "precision_weighted": prec, "recall_weighted": rec, "f1_weighted": f1v, "auc_macro_ovr": roc_auc}
