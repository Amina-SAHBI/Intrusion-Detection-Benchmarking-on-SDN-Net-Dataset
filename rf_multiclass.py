...
Random Forest-based 7-variant benchmarking script for multiclass intrusion detection on SDN-Net.

Features:
 - 7 experiment variants preserved (Unbalanced, RF-FS, IG-FS, SMOTE, SMOTE+RF-FS, SMOTE+IG-FS, SMOTE-Tomek)
 - Multiclass RandomForest (predict_proba)
 - Metrics: accuracy, weighted precision/recall/f1, macro OVR AUC when available
...
def rf_feature_importances(X_train, y_train, n_estimators=200):
    rf = RandomForestClassifier(n_estimators=n_estimators, random_state=RND, n_jobs=-1)
    rf.fit(X_train, y_train)
    importances = pd.Series(rf.feature_importances_, index=X_train.columns)
    return importances.sort_values(ascending=False)

def rf_feature_selection(X_train, y_train, k, n_estimators=200):
    importances = rf_feature_importances(X_train, y_train, n_estimators=n_estimators)
...
def prepare_for_multiclass(csv_path):
...
def train_rf_display_multiclass(
    name, X_train_df, X_test_df, y_train, y_test, features, class_names,
    algo_name="RandomForest-Multiclass", n_estimators=200, n_jobs=-1,
    ...
    if class_weight is not None:
        clf = RandomForestClassifier(n_estimators=n_estimators, random_state=RND, n_jobs=n_jobs, class_weight=class_weight)
    else:
        clf = RandomForestClassifier(n_estimators=n_estimators, random_state=RND, n_jobs=n_jobs)
    ...
    if show_fs_info:
        try:
            rf_imp = rf_feature_importances(X_train_df, y_train, n_estimators=n_estimators)
        except Exception:
    ...
    plt.tight_layout(); plt.show()
    try:
        plot_learning_curve_for_model(RandomForestClassifier(n_estimators=n_estimators, random_state=RND, n_jobs=n_jobs), X_train_df, y_train, title=f"Learning Curve ({name})", cv=5, n_jobs=n_jobs)
    except Exception:
        pass
    if save_outputs:
        try:
            model_save_dir = model_dir / f"rf_multiclass_model_{timestamp()}"
            clf.save = None  # not needed but avoid potential issues
            joblib.dump(clf, model_save_dir.with_suffix('.joblib'))
        except Exception:
            try:
        ...
        try:
            ...
            pass
        # save features and mapping
        if features is not None:
            pd.Series(list(features), name="feature").to_csv(model_dir / f"features_{timestamp()}.csv", index=False)
        try:
            with open(model_dir / f"classes_{timestamp()}.json", "w", encoding="utf8") as f:
                json.dump({"classes": class_names}, f, indent=2, ensure_ascii=False)
        except Exception:
            pass
        # metrics
        metrics = {"algorithm": algo_name, "n_estimators": int(n_estimators), "n_classes": int(len(class_names)), "accuracy": float(acc), "precision_weighted": float(prec), "recall_weighted": float(rec), "f1_weighted": float(f1v), "auc_macro_ovr": float(roc_auc) if roc_auc is not None else None}
        ...
        with open(model_dir / f"classification_report_{timestamp()}.txt", "w", encoding="utf8") as f:
            f.write(report)
        try:
            fig.savefig(model_dir / f"confusion_roc_{timestamp()}.png", dpi=150); plt.close(fig)
        except Exception:
            pass
    if pause_between:
        try: input("Press Enter to continue...") 
        except Exception: time.sleep(2)
    return {"config": name, "algorithm": algo_name, "n_estimators": int(n_estimators), "n_classes": int(len(class_names)), "accuracy": acc, "precision_weighted": prec, "recall_weighted": rec, "f1_weighted": f1v, "auc_macro_ovr": roc_auc}
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="RandomForest-Multiclass")
    parser.add_argument("--n_estimators", type=int, default=200)
    parser.add_argument("--n_jobs", type=int, default=-1)
    parser.add_argument("--class_weight", action="store_true")
    args = parser.parse_args(argv)

    X_full, y_full, class_names = prepare_for_multiclass(args.csv)
    print("Dataset shape:", X_full.shape, "n_classes:", len(class_names))
    X_train_raw, X_test_raw, y_train, y_test = train_test_split(X_full, y_full, test_size=0.33, random_state=RND, stratify=y_full)

    results = []
    results.append(train_rf_display_multiclass("Model1_Unbalanced_Full", X_train_raw, X_test_raw, y_train, y_test, X_full.columns, class_names, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))
    sel2 = rf_feature_selection(X_train_raw, y_train, args.k, n_estimators=args.n_estimators)
    results.append(train_rf_display_multiclass("Model2_Unbalanced_RF-FS", X_train_raw[sel2], X_test_raw[sel2], y_train, y_test, sel2, class_names, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    sel3, ig_scores = ig_feature_selection_with_scores(X_train_raw, y_train, k=args.k)
    results.append(train_rf_display_multiclass("Model3_Unbalanced_IG-FS", X_train_raw[sel3], X_test_raw[sel3], y_train, y_test, sel3, class_names, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    ...
    results.append(train_rf_display_multiclass("Model4_SMOTE_Full", X_train_m4, X_test_raw, y_train_m4, y_test, X_full.columns, class_names, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))
    sel5 = rf_feature_selection(X_train_m4, y_train_m4, args.k, n_estimators=args.n_estimators)
    results.append(train_rf_display_multiclass("Model5_SMOTE_RF-FS", X_train_m4[sel5], X_test_raw[sel5], y_train_m4, y_test, sel5, class_names, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    sel6, ig_scores_m4 = ig_feature_selection_with_scores(X_train_m4, y_train_m4, k=args.k)
    results.append(train_rf_display_multiclass("Model6_SMOTE_IG-FS", X_train_m4[sel6], X_test_raw[sel6], y_train_m4, y_test, sel6, class_names, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    ...
    results.append(train_rf_display_multiclass("Model7_SMOTETomek_Full", X_train_m7, X_test_raw, y_train_m7, y_test, X_full.columns, class_names, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))
