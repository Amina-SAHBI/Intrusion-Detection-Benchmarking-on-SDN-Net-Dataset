...
Decision Tree multiclass 7-variant benchmarking script for intrusion detection on the SDN-Net dataset.

This adapts dt_binary.py to multiclass classification by:
 - using DecisionTreeClassifier for multiclass (softmax-like predict_proba available)
...
Usage:
  python dt_multiclass.py --csv ../SDN-Net.csv --k 30 --save
...
def train_dt_display_multiclass(
    name,
    X_train_df,
    X_test_df,
    y_train,
    ...
    features,
    class_names,
    algo_name="DecisionTree-Multiclass",
    save_outputs=False,
    outdir="outputs",
    pause_between=False,
    show_fs_info=False,
    ...
    """
    Train DecisionTree for multiclass, display classification report, confusion matrix and per-class ROC.
    """
    stamp = timestamp()
    ...
    # Class weights (multiclass) - optional, used to balance training if requested
    try:
    ...
    # Fit (DecisionTree supports class_weight)
    if class_weight:
        clf = DecisionTreeClassifier(random_state=RND, class_weight=class_weight)
    else:
        clf = DecisionTreeClassifier(random_state=RND)
    ...
    # Probabilities for multiclass (if available)
    y_prob = None
...
def load_and_prepare_multiclass(csv_path):
    ...
    # One-hot encode object cols except the label columns
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="DecisionTree-Multiclass", help="Algorithm name label to include in results (default: DecisionTree-Multiclass)")
