...
k-Nearest Neighbors-based 7-variant benchmarking script for binary intrusion
...
for binary classification. The `algo_name` label is included in printouts,
...
Features:
 ...
 - "algorithm" field saved in metrics and summary (default: "KNN")
 - Optional saving (--save) and pausing between models (--pause)

Usage:
  python knn_binary_fixed.py --csv ../SDN-Net.csv --k 30
  python knn_binary_fixed.py --csv ../SDN-Net.csv --k 30 --save --outdir outputs --pause
...
def train_knn_display(
    ...
    """
    Train classifier (k-Nearest Neighbors by default), display classification
    report, confusion matrix and ROC inline.

    algo_name: string to display and include in results (e.g. 'KNN')
    n_neighbors: number of neighbours (k) for KNN
    ...
    try:
        if hasattr(pipeline, "predict_proba"):
            score_for_roc = pipeline.predict_proba(X_test_df)[:, 1]
    except Exception:
        score_for_roc = None
    if score_for_roc is None:
        try:
            score_for_roc = pipeline.decision_function(X_test_df)
        except Exception:
            score_for_roc = None
    if score_for_roc is None:
        # Fallback (discrete predictions)
    ...
    if save_outputs:
        # save the entire pipeline (scaler + model) and also the scaler separately
...
def main(argv=None):
    ...
    parser.add_argument(
        "--algo",
        default="KNN",
        help="Algorithm name label to include in results (default: KNN)"
    )
    parser.add_argument(
        "--n_neighbors",
        type=int,
        default=5,
        help="Number of neighbours (k) for KNN (default: 5)"
    )
