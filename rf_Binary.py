...
def train_rf_display(
    ...
    if class_weight_option:
        try:
            ...
            # sklearn expects dict for class_weight
    ...
    if show_fs_info and isinstance(features, (list, pd.Index, np.ndarray)):
        ...
        try:
            rf_imp = rf_feature_importances(Xtr_fs, y_train, n_estimators=n_estimators)
    ...
    axes[1].set_title(f"ROC Curve - {name} ({algo_name})"); axes[1].set_xlabel("False Positive Rate"); axes[1].set_ylabel("True Positive Rate")
    axes[1].legend(loc='lower right'); plt.tight_layout(); plt.show()

    # Learning curve
    try:
        plot_learning_curve_for_model(RandomForestClassifier(n_estimators=n_estimators, random_state=RND, n_jobs=n_jobs), X_train_df, y_train, title=f"Learning Curve ({name} - {algo_name})", cv=5, n_jobs=n_jobs)
    except Exception as e:
    ...
    if save_outputs:
        ...
        try:
            ...
            auc_value = None
        metrics = {"algorithm": algo_name, "n_estimators": int(n_estimators), "accuracy": float(acc), "precision": float(prec), "recall": float(rec), "f1": float(f1v), "auc": auc_value}
    ...
    return {"config": name, "algorithm": algo_name, "n_estimators": int(n_estimators), "accuracy": acc, "precision": prec, "recall": rec, "f1": f1v, "auc": roc_auc}
...
def load_and_prepare(csv_path):
    ...
    # Ensure Attack Type or Class present
...
def main(argv=None):
    ...
    parser.add_argument("--algo", default="RandomForest", help="Algorithm name label to include in results (default: RandomForest)")
    parser.add_argument("--n_estimators", type=int, default=200, help="Number of trees (default: 200)")
    parser.add_argument("--n_jobs", type=int, default=-1, help="n_jobs for RF (default: -1)")
    parser.add_argument("--class_weight", action="store_true", help="Use class_weight='balanced' (default: False)")
    ...
    results.append(train_rf_display("Model1_Unbalanced_Full", X_train_raw, X_test_raw, y_train, y_test, X_full.columns, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))

    sel2 = rf_feature_selection(X_train_raw, y_train, args.k, n_estimators=args.n_estimators)
    results.append(train_rf_display("Model2_Unbalanced_RF-FS", X_train_raw[sel2], X_test_raw[sel2], y_train, y_test, sel2, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))

    sel3, ig_scores_full = ig_feature_selection_with_scores(X_train_raw, y_train, k=args.k)
    results.append(train_rf_display("Model3_Unbalanced_IG-FS", X_train_raw[sel3], X_test_raw[sel3], y_train, y_test, sel3, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    ...
    results.append(train_rf_display("Model4_SMOTE_Full", X_train_m4, X_test_raw, y_train_m4, y_test, X_full.columns, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))

    sel5 = rf_feature_selection(X_train_m4, y_train_m4, args.k, n_estimators=args.n_estimators)
    results.append(train_rf_display("Model5_SMOTE_RF-FS", X_train_m4[sel5], X_test_raw[sel5], y_train_m4, y_test, sel5, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))

    sel6, ig_scores_m4 = ig_feature_selection_with_scores(X_train_m4, y_train_m4, k=args.k)
    results.append(train_rf_display("Model6_SMOTE_IG-FS", X_train_m4[sel6], X_test_raw[sel6], y_train_m4, y_test, sel6, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause, show_fs_info=True, top_k_fs=args.k))
    ...
    results.append(train_rf_display("Model7_SMOTETomek_Full", X_train_m7, X_test_raw, y_train_m7, y_test, X_full.columns, algo_name=args.algo, n_estimators=args.n_estimators, n_jobs=args.n_jobs, class_weight_option=args.class_weight, save_outputs=args.save, outdir=args.outdir, pause_between=args.pause))
